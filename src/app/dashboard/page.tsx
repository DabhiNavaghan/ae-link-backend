    ...
    <div style={{ fontFamily: 'var(--font-body)' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          {/* Store links for the selected app */}
          {(() => {
            const currentApp = apps.find((a) => a.id === selectedAppId);
            const playUrl = currentApp?.androidStoreUrl;
            const appUrl  = currentApp?.iosStoreUrl;
            if (!playUrl && !appUrl) return null;
            return (
              <>
                {playUrl && (
                  <a
                    ...
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 6, textDecoration: 'none', color: 'var(--color-success)', borderColor: 'var(--color-success)' }}
                  >
